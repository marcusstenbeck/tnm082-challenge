		  ...
		  //koppla ihop knappen med xml:en
	        ...
	        tbg.setOnClickListener(new OnClickListener()
	        {
				public void onClick(View v)
				{
					...
					if(tbg.isChecked())
					{
						currentGroup.addUser(User.dummyUser()); //ska komma frÂn session (anv‰nd dummy-user)
						//Mittcentrerad popup som sager Uppdraget Accepterat
						Toast toast = Toast.makeText(getBaseContext()," GÂtt med i gruppen", Toast.LENGTH_LONG);
					...
					{
						currentGroup.removeUser(User.dummyUser()); //tar av nÂn anledning inte bort frÂn databasen
						//Mittcentrerad popup som sager Uppdraget Accepterat
						Toast toast = Toast.makeText(getBaseContext()," GÂtt ur gruppen", Toast.LENGTH_LONG);
						toast.setGravity(Gravity.CENTER, 0, 0);
						toast.show();
				    }
				}
